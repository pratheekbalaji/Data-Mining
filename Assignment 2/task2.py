...
all_items =  rdd.filter(lambda x: x != header).map(lambda x: x.split(',')).map \
    (lambda x: ((x[0]), (x[1]))).groupByKey().mapValues(set).map(lambda x: x[1]).filter \
    (lambda x: len(x ) >threshold).persist()
